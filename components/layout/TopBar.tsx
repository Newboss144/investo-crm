...
export default function TopBar() {
  ...
  const fetchAlerts = async () => {
    ...
    try {
      ...
      for (const p of policies) {
        ...
        // Rule 2: Matured Status
        if (p.status === 'matured') {
          const id = `matured-${p.id}`;
          if (!dismissed.includes(id)) {
            list.push({
              id,
              title: 'Policy Matured',
              message: `Policy ${p.policyNo} for ${p.customerName} has matured.`,
              customerId: p.customerId,
              type: 'status',
            });
          }
        ...
        if (p.status === 'lapsed') {
          ...
          if (!dismissed.includes(id)) {
            list.push({
              id,
              title: 'Policy Lapsed',
              message: `Policy ${p.policyNo} for ${p.customerName} has lapsed.`,
              customerId: p.customerId,
              type: 'status',
            });
          }
  ...
  return (
    <header className="h-14 lg:h-16 bg-slate-900 border-b border-slate-800 flex items-center px-3 lg:px-6 gap-2 lg:gap-4 flex-shrink-0 pl-12 lg:pl-6">
      {/* Search bar */}
      <div ref={searchContainerRef} className="flex-1 max-w-xl relative min-w-0">
      ...
      {/* Notifications bell */}
      <div ref={notificationsRef} className="relative ml-auto flex items-center gap-1 lg:gap-2 flex-shrink-0">
        <div className="relative">
          <button
          ...
          <AnimatePresence>
            {showNotifications && (
              <motion.div
                ...
                transition={{ duration: 0.15 }}
                className="absolute right-0 mt-3 w-[calc(100vw-2rem)] sm:w-80 max-w-sm bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl z-50 overflow-hidden"
              >
